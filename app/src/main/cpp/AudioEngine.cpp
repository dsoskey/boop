...
void AudioEngine::setToneOn(double frequency) {
    int32_t sampleRate = AAudioStream_getSampleRate(stream_); // Do I have to set the sample rate every time?
    oscillator_.setFrequency(sampleRate, frequency);
    oscillator_.setWaveOn(true);
}

void AudioEngine::setToneOff() {
    oscillator_.setWaveOn(false);
}

void AudioEngine::setWave(WaveGenerator* waveGenerator) {
    oscillator_.setWave(waveGenerator);
}
